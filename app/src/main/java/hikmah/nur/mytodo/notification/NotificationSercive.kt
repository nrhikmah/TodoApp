...
import hikmah.nur.mytodo.ui.TodoListActivity
import hikmah.nur.mytodo.utils.Constants
import java.util.*

class NotificationSercive : IntentService(Constants.INTENT_SERVICE_NAME) {
    ...
    override fun onHandleIntent(intent: Intent?) {
        ...
        if (intent != null && intent.extras != null) {
            timestamp = intent.extras!!.getLong("timestampt")
        }

        if (timestamp > 0) {
            val context =this.applicationContext
            var notificationManager: NotificationManager= getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
            val notifyIntent = Intent(this, TodoListActivity::class.java)
    ...
    companion object {
        const val CHANNEL_ID = "hikmah.nur.mytodo.notification.CHANNEL_ID"
